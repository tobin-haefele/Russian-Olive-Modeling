...
russian_olive_df <- russian_olive_df %>%
  dplyr::select(PA, X, Y, REAP, FROST_FREE_DAYS, ANNUAL_PRECIP,
                ...
                SOIL_BULK_DENSITY, LEVEL1, LEVEL2)
...
glm_test_table$Pred_Class <- ifelse(glm_test_table$Pred > 0.5, 1, 0)

# Confusion matrix components
TP <- sum(glm_test_table$Pred_Class == 1 & glm_test_table$PA == 1)
TN <- sum(glm_test_table$Pred_Class == 0 & glm_test_table$PA == 0)
FP <- sum(glm_test_table$Pred_Class == 1 & glm_test_table$PA == 0)
FN <- sum(glm_test_table$Pred_Class == 0 & glm_test_table$PA == 1)

# Calculate metrics
glm_sensitivity <- TP / (TP + FN)
glm_specificity <- TN / (TN + FP)
glm_accuracy    <- (TP + TN) / (TP + TN + FP + FN)
...
#get predictions 
gam_pred <- prediction(gam_test_table$Pred, gam_test_table$PA)

#create ROC
gam_roc_curve <- performance(gam_pred, measure = "tpr", x.measure = "fpr")

#create auc
gam_auc_value <- performance(gam_pred, measure = "auc")@y.values[[1]]

# predict class labels using threshold 0.5
gam_test_table$Pred_Class <- ifelse(gam_test_table$Pred > 0.5, 1, 0)

# Confusion matrix components
TP <- sum(gam_test_table$Pred_Class == 1 & gam_test_table$PA == 1)
TN <- sum(gam_test_table$Pred_Class == 0 & gam_test_table$PA == 0)
FP <- sum(gam_test_table$Pred_Class == 1 & gam_test_table$PA == 0)
FN <- sum(gam_test_table$Pred_Class == 0 & gam_test_table$PA == 1)

# Calculate metrics
gam_sensitivity <- TP / (TP + FN)
gam_specificity <- TN / (TN + FP)
gam_accuracy    <- (TP + TN) / (TP + TN + FP + FN)  # total correct / total samples
...
model_metrics <- rbind(
  ...
  data.frame(Model = "gam",
             ...
             Sensitivity = gam_sensitivity,
             Specificity = gam_specificity)
)
...
for (k in seq_len(length(folds))) {
  ...
  rf_test_table$Pred[test_ids] <- predict(
    rf_model,
    test_data,
    type = "prob"
  )[, 2]
...
hist(rf_test_table$Pred)

# predict class labels using threshold 0.5
rf_test_table$Pred_Class <- ifelse(rf_test_table$Pred > 0.5, 1, 0)

# Confusion matrix components
TP <- sum(rf_test_table$Pred_Class == 1 & rf_test_table$PA == 1)
TN <- sum(rf_test_table$Pred_Class == 0 & rf_test_table$PA == 0)
FP <- sum(rf_test_table$Pred_Class == 1 & rf_test_table$PA == 0)
FN <- sum(rf_test_table$Pred_Class == 0 & rf_test_table$PA == 1)

# Calculate metrics
rf_sensitivity <- TP / (TP + FN)
rf_specificity <- TN / (TN + FP)
rf_accuracy    <- (TP + TN) / (TP + TN + FP + FN)  # total correct / total samples
...
#pdp plot of random forest variables
pd <- partial(rf_model, pred.var = "LEVEL2", train = st_drop_geometry(rf_test_table), )

plotPartial(pd)

ggplot(pd, aes(x = LEVEL2, y = yhat)) +
  geom_col(fill = "steelblue") +
  labs(title = "Partial Dependence for LEVEL1",
       x = "LEVEL2 Category",
       y = "y-hat") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

vip(rf_model, aesthetics = list(fill = "steelblue", color = "black", alpha = 0.8))
...
# predict class labels using threshold 0.5
gbm_preds$Pred_Class <- ifelse(gbm_preds$GBM_Pred > 0.5, 1, 0)

# Confusion matrix components
TP <- sum(gbm_preds$Pred_Class == 1 & gbm_preds$PA == 1)
TN <- sum(gbm_preds$Pred_Class == 0 & gbm_preds$PA == 0)
FP <- sum(gbm_preds$Pred_Class == 1 & gbm_preds$PA == 0)
FN <- sum(gbm_preds$Pred_Class == 0 & gbm_preds$PA == 1)

# Calculate metrics
gbm_sensitivity <- TP / (TP + FN)
gbm_specificity <- TN / (TN + FP)
gbm_accuracy    <- (TP + TN) / (TP + TN + FP + FN)  # total correct / total samples
...
model_metrics <- rbind(
  ...
  data.frame(Model = "gbm",
             ...
             Sensitivity = gbm_sensitivity,
             Specificity = gbm_specificity)
)
```
...
#plot points based on predicted value
ggplot(rf_test_table) +
  geom_sf(aes(color = Pred), size = 1, alpha = 0.8) +
  scale_color_viridis_c(option = "plasma") +
  labs(title = "Predicted Probability of Russian Olive Presence",
       color = "Prediction") +
  theme_minimal()

ggplot(rf_test_table, aes(x = SOIL_PH, y = FROST_FREE_DAYS, color = Pred)) +
