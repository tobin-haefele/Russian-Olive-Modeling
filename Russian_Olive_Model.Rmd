...
# Plot bar plots
categorical_vars_long <- gather(categorical_vars, key = "variable", value = "value")

#
ggplot(categorical_vars_long, aes(x = value)) +
  geom_bar() +
  facet_wrap(~variable, scales = "free_x") +
  theme_minimal() +
...
for (k in seq_along(folds)) {
  ...
  # Fit logistic regression
  gam_model <- gam(
    PA ~ s(FROST_FREE_DAYS) + s(ANNUAL_PRECIP) + s(SUMMER_AVG_MAXTEMP) + 
        s(WINTER_AVG_MINTEMP) + s(SOIL_PH) + s(SOIL_BULK_DENSITY) + 
        NEAR_WATER_150m + 
        LEVEL1 + LEVEL2,
    data = train_data,
    family = binomial,
