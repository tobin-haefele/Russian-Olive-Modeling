...
russian_olive_df <- russian_olive_df %>%
  dplyr::select(
    PA, X, Y, REAP, FROST_FREE_DAYS, ANNUAL_PRECIP,
...
numeric_vars <- russian_olive_df %>%
  dplyr::select(
    REAP, FROST_FREE_DAYS, ANNUAL_PRECIP,
    SUMMER_AVG_MAXTEMP, WINTER_AVG_MINTEMP, SOIL_PH,
    SOIL_BULK_DENSITY
  )
...
# select presence absence and land cover variables for plotting
...
evaluate_binary_classifier(
  df = st_drop_geometry(glm_test_table),
  pred_col = Pred,
  truth_col = PA,
  model_name = "glm",
  threshold = 0.5,
  auc_value = glm_auc_value
)

# Bind to model_metrics tibble
...
#use function to get result
evaluate_binary_classifier(
  df = st_drop_geometry(gam_test_table),
  pred_col = Pred,
  truth_col = PA,
  model_name = "gam",
  threshold = 0.5,
  auc_value = gam_auc_value
)
...
#use function to get result
evaluate_binary_classifier(
  df = st_drop_geometry(gbm_preds),
  pred_col = GBM_Pred,
  truth_col = PA,
  model_name = "gbm",
  threshold = 0.5,
  auc_value = gbm_auc_value
)
...
ggplot(rf_test_table) +
  ...
  labs(
    title = "Predicted Probability of Russian Olive Presence",
    color = "Prediction"
  ) +
  theme_minimal()
...
ggplot(pd, aes(x = LEVEL2, y = yhat)) +
  ...
  labs(
    title = "Partial Dependence for LEVEL1",
    x = "LEVEL2 Category",
    y = "y-hat"
  ) +
  theme_minimal() +
